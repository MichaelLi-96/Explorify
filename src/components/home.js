...
class Home extends Component {
  ...
  loadArtists = () => {
    ...
    for (let i = 0; i < this.state.artists.length; i++) {
      ...
      artists.push(<ArtistPreview key={artist._id} artistId={artist._id} />);
    }
    return artists;
  };
  ...
  loadTracks = () => {
    ...
    for (let i = 0; i < shuffledSongs.length; i++) {
      ...
      songs.push(<SongPreview key={song._id} songId={song._id} />);
    }
    return songs;
  };
    ...
    return (
      <div id="home">
        ...
        <div id="homeListsContainer">
          ...
          <div className="homeListContainer">{this.loadArtists()}</div>
          <div className="homeSubtitle">Featured Tracks</div>
          <div className="homeListContainer">{this.loadTracks()}</div>
        </div>
      </div>
    );
  }
